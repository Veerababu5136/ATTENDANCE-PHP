...
if (isset($_GET['download_excel'])) {
    ...
    $sheet->setCellValue('A1', 'S NO')
          ->setCellValue('B1', 'INTERN ID')
          ->setCellValue('C1', 'NAME')
          ->setCellValue('D1', 'EMAIL')
          ->setCellValue('E1', 'BATCH NUMBER')
          ->setCellValue('F1', 'CONTACT');

    // Query to fetch data
    $query = "SELECT dss_id, name,email,batch_no,mobile_number FROM interns ORDER BY dss_id ASC";
    ...
    while ($row = mysqli_fetch_assoc($query_runner)) {
        $sheet->setCellValue('A' . $row_num, $sno)
              ...
              ->setCellValue('D' . $row_num, $row['email'])
              ->setCellValue('E' . $row_num, $row['batch_no'])
              ->setCellValue('F' . $row_num, $row['mobile_number']);
        $sno++;
        $row_num++;
    }
                      ...
                      if (mysqli_num_rows($query_runner) > 0) {
                          ...
                          while ($row = mysqli_fetch_assoc($query_runner)) {
                              echo "<tr>
                                     ...
                                     <td data-label='Batch Name'>{$row['mobile_number']}</td>
